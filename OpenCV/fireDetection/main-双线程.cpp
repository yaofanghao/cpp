/*
 * @Author: yao fanghao
 * @Date: 2023-06-19 16:42:06
 * @LastEditTime: 2023-06-19 17:44:28
...
int receive_preiod = 4; //  每间隔 recieve_preiod * flag 接收一次串口的数据
double fps = 25.0;
int CapWidth = 1280;
int CapHeight = 960;
int ellipse_low = 5;  // There should be at least 5 points to fit the ellipse
std::vector<double> receive_num;  // 存放从STM32接收到数据
std::vector<double> processing_result; // 存放每帧图片图片后的特征值（圆形度、偏心率、熵）
...
// 共享缓冲区
// queue<int> buffer;
std::vector<double> processing_result;
...
int main(int argc, char** argv)
{
	...
	// 级联分类器方法，未并入该项目 -- 详见 fireCascade.cpp
	case 'e':{
