    ...
    while (true)
    {
        ...
        if (mColorStream.isValid())
        {
            ...
            if (mColorStream.readFrame(&mColorFrame) == STATUS_OK)
            {
                ...
                // 手部检测示例          
                vector<Point2f> myHandPoint;
                myHandPoint = detectAndDisplayHand(cImageBGR); // 8.17 返回手部中心点坐标

                for (int n = 0; n < myHandPoint.size(); n++)  // 读取出手部中心点坐标 
                {
                    cout << myHandPoint[n].x << "---" << myHandPoint[n].y << endl;
                    //cout << int(myFacePoint[n].x) << "---" << int(myFacePoint[n].y) << endl;
                    if (mDepthStream.readFrame(&mDepthFrame) == STATUS_OK) // 读取人脸中心点对应深度值
                    {
                        int hand_x = myHandPoint[n].x;
                        int hand_y = myHandPoint[n].y;

                        DepthPixel* handCenterPixel = (DepthPixel*)((char*)mDepthFrame.getData() +
                            (hand_y * mDepthFrame.getStrideInBytes())) +
                            (hand_x * mDepthFrame.getStrideInBytes() / mDepthFrame.getWidth());
                        int dX, dY, dZ;
                        dX = hand_x;
                        dY = hand_y;
                        dZ = *handCenterPixel;
                        ...
                        //Mat FaceCenter;
                        circle(cImageBGR, myHandPoint[n], 5, Scalar(0, 0, 255), 2); // 给中心点画圆
                        putText(cImageBGR, to_string(wZ), myHandPoint[n], FONT_HERSHEY_PLAIN, 2, Scalar(0, 0, 255));
                        cv::imshow("Hand Center", cImageBGR);
                    }
                    else
                    {
